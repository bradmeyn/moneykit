...
export const addIncome = form(incomeSchema, async (data) => {
	const user = await getCurrentUser();
	if (!user) error(401, 'Unauthorized');
...
export const updateIncome = form(incomeSchema.extend({ id: z.string() }), async (data) => {
	const user = await getCurrentUser();
	if (!user) error(401, 'Unauthorized');
	...
	await db
		.update(incomeTable)
		.set({ name: data.name, amount: Math.round(data.amount * 100), frequency: data.frequency, category: data.category || null })
